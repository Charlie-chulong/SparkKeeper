...
| 版本更新 | 3.0.2 起推荐双击用户级安装 EXE 安装或升级；保留 ZIP 离线更新和数据库迁移保护 |
...
1. 打开 [最新 Release](https://github.com/Charlie-chulong/SparkKeeper/releases/latest)，**推荐下载 `SparkKeeper-3.0.2-Setup.exe`**（以后选择对应新版的 `Setup.exe`），并取得同页 SHA-256 校验文件。**不要把 GitHub 的 `Source code` 压缩包当作 Windows 程序。**
...
**ZIP 备用方案：** 下载 `SparkKeeper-3.0.2-win64.zip`，完整解压，将包内 `SparkKeeper` 文件夹放到当前用户可写的固定目录，再运行其中的 `SparkKeeper.exe`。不要只复制 EXE，也不要直接把 ZIP 合并覆盖到安装器管理的目录。
...
1. 在旧版停用每日计划并保存，等待发送、扫描、登录及独立计划任务结束，正常关闭所有旧窗口。
...
3. 若提示程序占用或计划仍启用，返回旧版正常结束操作、停用并保存计划后再重试；安装器不会强杀进程。
4. 安装后运行新版，核对好友、登录状态和计划，再人工启用并保存计划；不会自动补跑。

安装器升级和卸载会校验已有程序的发行清单及 SHA-256；文件损坏、手工替换的组件或未知额外文件会导致拒绝继续，不会直接清除未知文件。没有 `release-manifest.json` 的旧便携目录不能原地覆盖，请迁入新的默认安装目录。出现错误时保留安装日志并反馈，不要混合新旧依赖或删除用户数据来绕过检查。

**首次从 ZIP 便携版迁入：** 按上述步骤退出便携版，再安装到默认用户级目录。同一 Windows 用户下继续使用 `%LOCALAPPDATA%\SparkKeeper` 中的已有数据，不要复制数据库或登录态到程序目录。首次迁入后必须在新版重新保存每日计划，使 Windows 任务中的绝对路径指向新 EXE；旧快捷方式和任务栏固定项也需改为新路径。不要再运行旧目录中的副本。

**安装版换位置：** 不要直接移动目录或在升级时指定另一目录。先停用并保存计划、正常退出，完成标准卸载（保留数据）后重新安装到新位置，再重新保存每日计划并更新快捷方式。

### 备用：继续使用 ZIP 离线更新

仅更新便携目录时，将新 ZIP 完整解压到**另外的临时目录**，双击新包根目录的 **`更新.cmd`** 并选择已有便携程序目录，不要直接合并覆盖旧文件；更新前同样必须停用计划并正常退出，更新后核对配置再人工启用计划。
...
先在程序中停用每日计划并保存，等待任务结束、正常退出，再从 Windows“已安装的应用”中卸载 SparkKeeper。卸载移除安装的程序文件，但**保留 `%LOCALAPPDATA%\SparkKeeper` 中的数据库、登录态、配置和历史**；卸载不是清空数据。安装、升级和卸载都不承诺自动无损回滚数据库；新版产生发送记录后，不要恢复旧数据库覆盖新记录。
...
**安装、更新或卸载提示文件占用怎么办？** 先停用并保存每日计划，等待任务正常结束并退出旧版，包括独立计划任务。不要强杀正在确认发送结果的程序，也不要只覆盖 EXE。
...
1. 修改唯一版本号和 `CHANGELOG.md`，完成回归、构建、安装/升级/卸载保留数据及 ZIP 解压更新验证；文档说明不等于已完成安装验收。
