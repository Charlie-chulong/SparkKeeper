...
- 管理计划任务。
- 展示历史、日志和待处理操作。
- 主界面采用左侧原生 `QPushButton` 导航（任务台、好友管理、发送历史、运行日志）和右侧 `QStackedWidget` 页面栈。非活动页由 Qt 管理可见性和焦点，不再手动移动到屏外。选中与悬停颜色来自当前色板，固定宽度左侧标记区分状态，不改变字体度量或造成位移。
- 主界面共用 `ui/theme.py`：以浅色/暗夜语义色板统一 Qt 调色板、字体和 QSS，以及圆角导航、页面标题、卡片、状态徽章、操作层级、表格斑马纹和空状态。卡片圆角约 14px，按钮和输入区域约 8px；界面由 Qt 缓冲绘制，不混用 Tk 控件，不使用截图遮罩或原生重绘补丁。
- 展开列表与菜单外框统一为约 10px 圆角，列表选中项为约 6px；下拉列表使用普通 item delegate 的 QSS 绘制，避免原生菜单绘制绕过 item 圆角。共享主题事件过滤器为浮层设置 Qt 窗口区域，并在 Qt 自身 Resize/StyleChange 后合并刷新，防止原生处理重新引入方角；延后回调绑定窗口生命周期，不依赖 Qt 私有类名。仅 `Qt.Popup` 下拉容器去除原生 `QFrame`，以选择框为父控件的提示框不按下拉容器处理。预览和消息窗口保留原生标题栏与窗口操作，圆润边框绘制在客户区内。
- 默认窗口为 `1120x780`，最小窗口为 `940x650`；卡片说明自动换行，宽表格使用 Qt 原生按需滚动条，最小尺寸仍保留操作入口、进度区域和状态栏。“本次进度”仅保留卡片外框，内部表格不重复加框；其他表格保留轻边框。
- 账号动作按状态互斥显示：未登录仅扫码登录，已登录仅清除登录；忙碌期间禁用。启用控件保留 `QCheckBox` 的鼠标和键盘语义，采用约 12 逻辑像素的圆形指示器，空心/实心及颜色按当前色板区分状态，不缩小文字点击区域。
- 切页先显示已保留的页面，再后台读取好友、历史或日志；每页最多一个读取任务，通过访问世代与刷新版本丢弃过期结果。表格按稳定行标识增量更新，保留未保存编辑、选择、焦点及滚动位置，不在切页时整表清空重建。
- 已保存好友仅展示启用状态和昵称，不展示抖音号或搜索词；以原生 `allColumnsShowFocus` 显示整行焦点，支持 Ctrl/Shift 多选及选择计数。操作栏使用“全选/取消选择”切换按钮和上下文菜单，按选择数量与结构化启用状态刷新可见动作，并统一遵守忙碌/关闭状态。启停与确认后的删除在数据库中按整组选中 ID 原子执行；任一成员失败回滚整组，有历史的删除对象仅停用并保留记录。
...
- 左侧导航底部“外观”下拉框提供“浅色模式”“暗夜模式”“跟随系统”，分别对应 `ThemeMode.LIGHT`/`DARK`/`SYSTEM`。偏好与实际色板分离，缺失/非法偏好默认浅色；系统 Unknown 也解析为浅色。
- 外观切换立即更新主界面、已打开的预览、菜单及语义状态色，忙碌时仍可操作。系统配色通知通过 Qt signal 在主线程处理，仅 SYSTEM 重新解析色板，固定模式不跟随，也不修改系统全局主题。
- 偏好异步写入 `app_meta.theme_mode`，关闭等待写入完成；保存失败恢复上次成功保存的偏好，并保留异常原因链与完整错误详情。
...
键值设置，包括 schema 版本、页面适配器、最后一次错过检查日期及 `theme_mode`（`light`/`dark`/`system`，缺失或非法值按浅色处理）。
...
- 昵称
- 个人主页地址（如可见）
- 名称搜索值（`search_query`，保存昵称/备注名，不在好友表展示）
- 头像地址（可选，仅辅助确认）
- 身份证据 JSON
- 是否启用
- 确认时间
...
火花批量导入与单个用户确认保存不同：`import_spark_contacts` 在一次 `BEGIN IMMEDIATE` 中核对扫描账号及登录时间，对已有稳定键/规范主页/可信单聊会话摘要匹配的记录保持不变，只插入停用的新目标。任何歧义或写入异常整体回滚；不按名称合并身份，不依赖用户抖音号。火花观察时间和当时状态保存在身份依据中，不表示状态永久有效。
...
展示层将事件、发送尝试及进度的机器状态映射为中文，覆盖列表与详情；数据库机器值不随展示翻译。好友昵称、消息正文等用户内容和原始异常、原因链、调用栈等诊断文本保留原文，不对整段文本做全局替换。
...
当前 `SCHEMA_VERSION = 4`。升级在既有备份与事务边界内移除 `targets.douyin_id`，规范主页及身份依据、重建稳定键，并令 `search_query = display_name`。`FriendCandidate`/`Target` 不再具有 `douyin_id` 字段；可靠 SDK `user_id`/`sec_uid` 不属于用户抖音号，不删除其身份用途。迁移保持原目标主键、启用状态及历史/审计/防重复外键关系；无法建立稳定身份、唯一键冲突或其他异常使事务整体回滚，不静默丢弃记录，也不引入旧号码专用隔离或重确认流程。
...
该流程不复用日常浏览器账号，也不采集或使用用户抖音号。身份依据只接受规范抖音个人主页或可信单聊 SDK conversation 摘要；DOM 任意会话属性、昵称和头像不能独立证明身份。
...
- 发送前按保存名称重新搜索，再用稳定身份核验并打开；同名不默认取首项。
- 聊天打开后再次比对可见标题与规范抖音主页或可信单聊 SDK conversation 摘要，证据必须绑定当前对象。
- 身份证据不足或不一致时返回 `target_ambiguous` 或 `target_identity_mismatch`，禁止输入文本。

### 9.1 火花只读扫描

- `automation/spark_scan.py` 遍历当前左侧聊天列表，逐段滚动并处理虚拟列表复用；任意 DOM 会话标记仅用于去重，不升级为可发送身份。与该行绑定、通过单聊及参与者核验的可信 SDK conversation 摘要可作为稳定身份依据。
...
- 对方可靠 SDK `participant.sec_uid` 映射到规范抖音个人主页，可信单聊 SDK conversation 摘要可提供稳定会话身份；SDK `user_id`/`sec_uid` 不是用户抖音号。扫描、搜索候选和当前聊天复核共用身份规则，无法取得强身份复核时拒绝发送，不回退为仅昵称匹配。
